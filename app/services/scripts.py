...
def generate_script_audio(db: Session, script: Script, settings: Settings) -> str:
    ...
    规范见 app/audio.py：`data/audio/script-{id}-{正文sha1前12位}.wav`，
    8kHz/16bit/mono。正文未变时命中同名文件（缓存）直接复用，不再调用
    Provider；正文变化时生成新文件并原子写入。失败时把原因写入
    `tts_error`，供话术页展示失败状态。
    """
    target = script_audio_path(script.id, script.body)
    if target.exists():
    ...
    if result.success and result.wav_path:
        install_wav(Path(result.wav_path), target)
        script.tts_status = "generated"
        script.wav_path = str(target)
        script.tts_error = ""
        db.add(script)
...
def script_audio_url(script: Script) -> str:
    """话术 WAV 位于 data/audio/ 下时返回试听 URL（/audio/<文件名>），否则空串。

    手动指定的外部路径（如 smoke test 用 WAV）不提供 Web 试听，仍可被
    aplay 播放，避免 Web 路由暴露 data/audio 之外的任意文件。
    """

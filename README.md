...
- **协议到期维系**：每日固定时段（默认 09:00）扫描协议到期前 N 天（默认
  提前 14 天）的业务，电话通知客户经理办理续签维系；续签/退网通过
  「客户维系登记」（`/daily-renewals`）提交变更申请走审核链路。
- **退网设备回收**：退网业务下未回收设备每日提醒网络维护责任人回收，
  回收完成通过「设备回收登记」（`/daily-recycles`）走变更申请与审核。
- **审核卡单提醒**：待审核的变更申请每天定时提醒审核人员处理。
- **短信同步通知**：扫描配置勾选「同步短信通知」且 `SMS_ENABLED=1` 时，通知
  任务同步生成短信（A7670E 直发，无需运营商网关），CallWorker 空闲时串行
  发送（与语音共用串口互斥），`/sms` 页查看发送记录。

通话对象是**运维工作人员**（客户经理、网络维护责任人、审核人员），不是
客户本人；通知任务由每日扫描自动生成。三类通知各固定一个配置与一份话术
（`/notification-settings` 统一管理），扫描**按负责人聚合待办**：一个负责人
名下全部到期业务 / 待回收设备 / 待审核卡单合并为一条通知，话术 `{{待办清单}}`
列出全部事项；扫描配置可勾选同步短信通知（A7670E 短信通道，Worker 空闲时
串行发送），管理员不再手动设置客户回访计划。

...
在 Rock Pi 3A 上实现一个内网使用的定期电话回访管理 demo 系统。系统通过 A7670E 4G 电话模块拨打电话，通过 Rock Pi 本地音频输出播放预生成话术音频，并通过 Web 后台管理客户、话术、回访计划和通话记录。
...
- 拨号后小于 10 秒结束，且没有 `VOICE CALL: BEGIN`：`rejected`。
- 10 到 80 秒结束，且没有 `VOICE CALL: BEGIN`：`cancelled_or_failed`。
- 大于等于 80 秒结束，且没有 `VOICE CALL: BEGIN`：`no_answer`。
